...
#[async_trait]
impl QueryExec for SqliteSession {
    async fn execute_sql(&self, sql: &str) -> Result<QueryOutput, DatabaseError> {
        if statement_returns_rows(sql) {
            ...
            return Ok(QueryOutput::Table(decode_sqlite_rows(rows, sql)));
        }
...
impl SchemaExec for SqliteSession {
    async fn describe_table(
        ...
        Err(DatabaseError::Unsupported(
            "sqlite schema exec is not implemented yet".into(),
        ))
    }
...
impl DriverSession for SqliteSession {
    ...
    fn capabilities(&self) -> Capabilities {
        Capabilities::for_kind(DatabaseKind::Sqlite)
    }
...
fn decode_sqlite_rows(rows: Vec<sqlx::sqlite::SqliteRow>, sql: &str) -> models::QueryPage {
    ...
    if has_locator {
        ...
        sqlite_preview_rows_to_paginated_page(rows, unknown_preview_source(), page_size, offset)
    } else if let Some((limit, offset)) = page_meta {
        sqlite_rows_to_paginated_page(rows, limit.saturating_sub(1) as u32, offset)
    } else {
        sqlite_rows_to_page(rows)
    }
}
